...
%   Detailed explanation goes here
    ...
    for i = 1 : numel(allTrials)
       if contains(allTrials{i}, [trial2remove '_'])
            ...
            if isfield(data.ID, model) && isfield(data.ID.(model), allTrials{i})
                data.ID.(model) = rmfield(data.ID.(model), allTrials{i});
            end
            if isfield(data.SO, model) && isfield(data.SO.(model), allTrials{i})
                data.SO.(model) = rmfield(data.SO.(model), allTrials{i});
            end
            if isfield(data.SO_Activation, model) && isfield(data.SO_Activation.(model), allTrials{i})
                data.SO_Activation.(model) = rmfield(data.SO_Activation.(model), allTrials{i});
            end
            if isfield(data.JRL, model) && isfield(data.JRL.(model), allTrials{i})
